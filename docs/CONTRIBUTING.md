...
AI use is acceptable, only if you are able to audit the code. This means that you must understand the code that is generated. Please ensure that any AI generated code is also sufficiently tested.
...
Please see the [developer guide](docs/DEVELOPER_GUIDE.md) for in-depth details about Obtanium's architecture. Please also update this when making relevant changes.

It is encouraged that all commits have their signature verified, that is signed. 
Please see the [relevant GitHub documentation](https://docs.github.com/en/authentication/managing-commit-signature-verification/about-commit-signature-verification#ssh-commit-signature-verification); this links to SSH key signing, but other signing is acceptable.
