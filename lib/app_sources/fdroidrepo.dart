    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final List<String> apkUrls = selectedReleases
            ...
            .cast<String>()
            .toList();
        // Fork addition (reproducible-build verification): a <binaries> element
        // on the selected release means the repo publishes a reproducible build
        // verified against the source; otherwise there is no such data.
        String releaseReproducibleStatus(dynamic release) =>
            release.querySelector('binaries') != null
            ? reproducibleBuildStatusVerified
            : reproducibleBuildStatusNoData;
        final String reproducibleStatus = selectedReleases.isNotEmpty
            ? releaseReproducibleStatus(selectedReleases[0])
            : reproducibleBuildStatusNoData;
        return APKDetails(
          selectedVersion,
          getApkUrlsFromUrls(apkUrls),
